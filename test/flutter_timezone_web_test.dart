...
void main() {
  ...
  test('web plugin uses real JS interop for timezone data', () async {
    ...
    final local = await plugin.handleMethodCall(const MethodCall('getLocalTimezone'));
    expect(local, isA<String>());
    expect((local as String).isNotEmpty, isTrue);

    final available = await plugin.handleMethodCall(const MethodCall('getAvailableTimezones'));
